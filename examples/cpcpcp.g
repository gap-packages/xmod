##  expt/c5c5c5.g, 24/10/20 

LoadPackage("xmod"); 

CpCpCpCat2Groups := function( p ) 

    local  G, genG, o, a, b, c, pooo, pooc, pocc, pcoc, pccc, pobo, 
           pbbo, paba, pabb, pabo, paac, paoc, pobc, pbbc, pabc, 
    ...
    c := genG[3]; 
    SetName( G, "G" ); 
    ## construct the projections needed for the cat1-groups 
    ...
    C1 := [ Cat1Group( pooo, pooo ),  ##  1
            ...
            Cat1Group( paba, pabo ),  ## 13 ~ 5
            Cat1Group( paac, paoc )   ## 14 ~ 5
          ]; 
    Print( C1, "\n" ); 
    ## when p is small check the isomorphisms listed above 
    if ( p < 4 ) then 
        for i in [7..14] do 
            A := C1[i]; 
            Print( i, " : " ); 
